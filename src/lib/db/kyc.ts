...
/**
 * kyc_identities data access. Written to by the Sumsub webhook, read by the
 * KYC gate in order creation and the asset pages.
 */
...
export async function getApprovedKycForUser(userId: string): Promise<KycIdentityRow | null> {
  ...
  const result = await db
    ...
    .limit(1)
    .maybeSingle();
  return unwrapMaybe(result, 'kyc.getApprovedKycForUser');
}
...
export async function upsertKycByExternalId(input: UpsertKycInput): Promise<KycIdentityRow> {
  ...
  if (input.externalId) {
    const found = await db
      ...
      .maybeSingle();
    existing = unwrapMaybe(found, 'kyc.upsertKycByExternalId');
  } else {
    const found = await db
      .from('kyc_identities')
      .select('*')
  ...
  if (existing) {
    const updated = await db
      ...
      .select('*')
      .single();
    return unwrap(updated, 'kyc.upsertKycByExternalId');
  }
  ...
  try {
    const inserted = await db
      ...
      .select('*')
      .single();
    return unwrap(inserted, 'kyc.upsertKycByExternalId');
  } catch (error) {
    ...
    const raced = await db
      ...
      .select('*')
      .single();
    return unwrap(raced, 'kyc.upsertKycByExternalId');
  }
}
