...
export default function Dashboard() {
    ...
    useEffect(() => {
        db.collection('BusinessUser').doc(params.userId).get().then((doc) => {
            if (doc.exists) {
    ...
    async function fetchData() {
        if (params.userId && assets.length && nfts.length && user) {
            const currentUserAssets = assets.filter((asset) => asset.minterId === params.userId)
            const salesWithNFTData: {
                asset: Asset;
                nft: NFT;
                nftOwner: NFTOwner;
            }[] = []
            currentUserAssets.map((asset) => {
                ...
                if (nft) {
                    salesWithNFTData.push({ asset, nft, nftOwner: { username: user.displayName, logo: user.logo } });
                }
            })
            setListings(salesWithNFTData.sort((a, b) => new Date(b.asset.createdAt).getTime() - new Date(a.asset.createdAt).getTime()))
            setLoading(false);
        }
    }
    useEffect(() => { fetchData() }, [assets, nfts, user]);
        ...
        <Box sx={{ backgroundColor: 'marketplace.background', minHeight: "100vh", display: "flex", flexDirection: "column" }}>
            ...
            <Container sx={{ flexGrow: 1 }}>
                <Box sx={{ mt: 5, display: 'flex', gap: 1.5, alignItems: "center" }}>
                    <Image src={user?.logo ?? "/svg/Avatar.svg"} width={0} height={0} sizes="100vw" alt='' className="rounded-full" style={{ width: '50px', height: '50px', objectFit: 'cover' }} />
                    <Typography style={{ fontWeight: 500 }} variant="h6" sx={{ color: 'navbar.primary' }}>{user?.legalName || user?.displayName}</Typography>
                </Box>
                <Box sx={{ mt: 2, display: 'flex', flexDirection: { xs: "column", sm: 'row' }, gap: { xs: 1, sm: 3 }, alignItems: { sm: "center" } }}>
                    {[
                        {
                            'label': 'Legal Name',
                        ...
                        <Box key={index}>
                            <Typography variant="subtitle2" sx={{ color: 'assetPurchase.documentRedirectSecondaryText' }}>{item.label}</Typography>
                            <Typography onClick={() => {
                                if (item.label === 'Phone Number') {
                                    window.open(`https://wa.me/${item.value}`, '_blank');
                                }
                            }} variant="subtitle1" sx={{ cursor: item.label === 'Phone Number' ? 'pointer' : 'default', color: 'portfolio.tableText' }}>{item.value}</Typography>
                        </Box>
                    ))}
                </Box>
                <Box sx={{ mt: 5, display: 'flex', justifyContent: { sm: 'center' }, gap: 1 }}>
                    {[
                        ...
                        },
                        {
                            ...
                            'value': listings.reduce((acc, item) => acc + item.nft.totalSupply, 0)
                        },
                        {
                            'label': 'Fractions Sold',
                            'value': listings.reduce((acc, item) => acc + (item.asset.initialSupply - item.asset.availableSupply), 0)
                        }
                    ].map((item, index) => {
                        return (
                            <Box key={index} sx={{ py: { xs: 3, sm: 4 }, borderRadius: "10px", px: { xs: 1.5, sm: 3 }, height: { xs: "177px", sm: "212px", horizontalTablet: "100%" }, backgroundColor: "navbar.background", border: 1, borderColor: "border", flexGrow: 1 }}>
                ...
                <Typography variant="h5" sx={{ color: 'navbar.primary', fontWeight: 500, mt: 5 }}>Tokenized Assets</Typography>
                {
                    loading ?
                        <>
                        ...
                        <>
                            <Box sx={{ display: 'flex', mt: 4, gap: 1, flexWrap: 'wrap' }}>
                                ...
                                {["All", ...categories.map(c => c.name).filter((key) => isNaN(Number(key)))].map((key, index) => (
                                    <Chip key={index} label={key} sx={{
                                        flexGrow: { xs: 1, sm: 0 },
                                        backgroundColor: selectedFilter === key ? 'marketplace.categoryFilter.background' : 'navbar.background',
                                        border: selectedFilter === key ? '' : 1,
                                        borderColor: selectedFilter === key ? '' : 'border',
                                        color: selectedFilter === key ? 'marketplace.categoryFilter.text' : 'marketplace.filterButtonText',
                                        '&:hover': {
                                            backgroundColor: selectedFilter === key ? 'marketplace.categoryFilter.background' : 'navbar.background',
                                            border: selectedFilter === key ? '' : 1,
                                            borderColor: selectedFilter === key ? '' : 'border',
                            ...
                            <Box sx={{ mt: { xs: 3, sm: 5, horizontalTablet: 7 }, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: { xs: 1, sm: 2.5 } }}>
                                {listings.filter((listing) => selectedFilter === "All" || listing.nft.metadata.assetClass === selectedFilter).map((item, index) => (
                                    <ListingCard key={index} item={item} />
                                ))}
                            </Box>
                            <Typography variant="h5" sx={{ mt: 5, fontWeight: 500, color: "navbar.primary" }}>Tokenized Assets History</Typography>
                                        ...
                                        {listings.map
                                            ((row, index) => (
                                                <StyledTableRow key={index}>
                                                    ...
                                                    {/* <StyledTableCell>{row.nft.metadata.location}</StyledTableCell>
                                                    <StyledTableCell>{row.nft.metadata.category}</StyledTableCell> */}
                                                    <StyledTableCell>{(row.asset.initialSupply - row.asset.availableSupply)}/{row.nft.totalSupply}</StyledTableCell>
                            ...
                            <Box onClick={() => setDisplayedLength(displayedLength + 5)}
                                sx={{ display: listings.length > displayedLength ? 'flex' : 'none', justifyContent: 'center', mt: 5, alignItems: 'center' }}>
