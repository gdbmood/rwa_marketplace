...
import { Box, Button, Container, Divider, Typography, Slider, CircularProgress, Modal, TextField } from '@mui/material'
...
export default function AssetSalePage() {
    ...
    const { mutate: sendAndConfirmApproveTx, error: approvetxError, isPending, isSuccess: isApproveTxSuccess } = useSendAndConfirmTransaction();
    ...
    const [fractionsToSell, setFractionsToSell] = useState<number>(1);
    const [fractionPrice, setFractionPrice] = useState<string>('1');
    const [remainingFractions, setRemainingFractions] = useState<number>(0);
    const [listing, setListing] = useState<{ purchase: Holding, nft: NFT }>();
    ...
    useEffect(() => {
        if (wallet && assets.length > 0) {
            db.collection('RetailUser').doc(wallet.address).collection('Holding').doc(params.purchaseId).get().then(doc => {
                if (doc.exists) {
                    const purchase = doc.data() as Holding;
                    const nft = nfts.find(nft => nft.nftId === Number(purchase.assetId));
                    if (nft) {
                        setListing({ purchase, nft });
                    }
                }
                else {
    ...
    useEffect(() => {
        if (listing) {
            setRemainingFractions(listing.purchase.quantity - listing.purchase.lockedQuantity);
        }
    }, [assets, listing, wallet]);
    useEffect(() => {
        if (error.trim() !== '') { setSeconds(0); return }

        if (isPending || isTxPending) {
            if (seconds === 0) {
                setSeconds(30);
            }
        }
    ...
    }, [seconds, isPending, isTxPending, error]);
    ...
    async function dbUpdate() {
        if (isTxSuccess) {
            if (wallet && listing && user) {
                try {
                    const assetRef = db.collection('Asset').doc(listing.nft.nftId.toString());
                    const assetData = (await assetRef.get()).data() as Asset;

                    await db.collection('Asset').doc(listing.nft.nftId.toString()).collection('Listing').add({
                        createdAt: new Date().toISOString(),
                        quantity: fractionsToSell,
                        pricePerFraction: Math.round(Number(fractionPrice) * 1e6) / 1e6,
                        currency: 'USDC',
                        state: ListingState.ACTIVE,
                        assetId: listing.nft.nftId,
                        listerId: wallet.address
                    });
                    await assetRef.update({
                        availableSupply: firebaseApp.firestore.FieldValue.increment(fractionsToSell),
                        pricePerFraction: Math.round(Number(fractionPrice) * 1e6) / 1e6 < assetData.pricePerFraction ? Math.round(Number(fractionPrice) * 1e6) / 1e6 : assetData.pricePerFraction,
                    });

                    const existingHolding = await db.collection('RetailUser').doc(wallet.address).collection('Holding').where('assetId', '==', listing.nft.nftId).get()
                    const existingHoldingData = existingHolding.empty ? null : existingHolding.docs[0].data() as Holding;
                    if (existingHoldingData) {
                        await db.collection('RetailUser').doc(wallet.address).collection('Holding').doc(existingHolding.docs[0].id).update({
                            lockedQuantity: firebaseApp.firestore.FieldValue.increment(fractionsToSell),
                        });
                    }
        ...
        if (isApproveTxSuccess) {
            if (listing) {
                const tx = prepareContractCall({
                    contract,
                    method: sellNFT,
                    params: [BigInt(listing.nft.nftId), BigInt(fractionsToSell), BigInt(Math.round(Number(fractionPrice) * 1e6))],
                });
                sendAndConfirmTx(tx);
            }
        }
    ...
    const sellFractions = async () => {
        if (!listing) return

        if (!wallet) { setError('Please connect your wallet'); return; }

        setError('');
        if (fractionsToSell < 1) { setError('Minimum amount is 1'); return; }
        if (fractionsToSell > remainingFractions) { setError('Amount exceeds your purchase'); return; }
        ...
        const existingHolding = await db.collection('RetailUser').doc(wallet.address).collection('Holding').where('assetId', '==', listing.nft.nftId).get()
        const existingHoldingData = existingHolding.empty ? null : existingHolding.docs[0].data() as Holding;
        if (existingHoldingData && existingHoldingData.lockedQuantity > 0) {
            setError('You have already listed this asset');
            return;
        }

        setSeconds(60);

        const tokenContract = getContractByAddress(listing.nft.erc20TokenAddress);
        const approveTx = prepareContractCall({
            contract: tokenContract,
            method: 'function approve(address spender, uint256 amount) returns (bool)',
            params: [contract.address, BigInt(fractionsToSell)],
        });
                        ...
                        <>
                            ...
                            >
                                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'marketplace.background', px: { xs: 3.5, sm: 5, verticalTablet: 12 }, py: 10, width: { xs: "100%", sm: "90%", horizontalTablet: "65%" }, overflowY: 'auto', height: 'auto', display: "flex", alignItems: "center", flexDirection: "column" }}>
                                    ...
                                    <Typography style={{ fontWeight: 500 }} sx={{ typography: { xs: "h6", sm: "h4" }, color: "navbar.primary", my: { xs: 3, sm: 5 } }}>The listing was successfully completed</Typography>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: { xs: 4, sm: 7.5 }, flexDirection: { xs: "column", sm: "row" } }}>
                                <Typography style={{ fontWeight: 500 }} sx={{ typography: { xs: "h6", sm: "h5" }, color: "assetPurchase.modal.heading" }}>LIST ASSET</Typography>
                            ...
                            <Box sx={{ display: 'flex', gap: 4, mt: 4, flexDirection: { xs: "column", verticalTablet: "row" } }}>
                                ...
                                <Box sx={{ flex: 1, width: "100%" }}>
                                    <Typography variant='h6' sx={{ color: "navbar.primary", fontWeight: 500 }}>What share of the asset do you want to list?</Typography>
                                    {
                                        ...
                                        <Box sx={{
                                            ...
                                            <Slider
                                                ...
                                                onChange={(e, value) => {
                                                    if (value === 0) {
                                                        setFractionsToSell(1)
                                                        setSliderValue(1)
                                                    }
                                                    else {
                                                        setSliderValue(Number(value))
                                                        setFractionsToSell(Number(value))
                                                    }
                                                }}
                                                ...
                                                sx={{
                                                    ...
                                                    '& .MuiSlider-track': {
                                                        ...
                                                        borderTopLeftRadius: '20px',
                                                        borderBottomLeftRadius: '20px',
                                                        borderTopRightRadius: sliderValue === remainingFractions ? '20px' : '0px',
                                                        borderBottomRightRadius: sliderValue === remainingFractions ? '20px' : '0px',
                                                    },
                                                    '& .MuiSlider-thumb': {
                                                        width: '0px',
                                                        height: '0px',
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 1, cursor: 'pointer', flexWrap: 'wrap' }}>
                                        {['1', '2', '5', '10', 'All'].map((i) => (
                                            <Box onClick={() => {
                                                setFractionsToSell(i === 'All' ? remainingFractions : Number(i) <= remainingFractions ? Number(i) : remainingFractions)
                                                setSliderValue(i === 'All' ? remainingFractions : Number(i) <= remainingFractions ? Number(i) : remainingFractions)
                                            }}
                                    ...
                                    <Box sx={{ mt: 4, mb: 2 }}>
                                        ...
                                        <Box sx={{ backgroundColor: "listingCard.background", px: 3, py: 1.5, mt: 2, display: "flex", justifyContent: "space-between" }}>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
                                                <svg onClick={() => {
                                                    if (fractionsToSell < remainingFractions) {
                                                        setFractionsToSell(fractionsToSell + 1)
                                                        setSliderValue(fractionsToSell + 1)
                                                    }
                                                ...
                                                <Typography variant='subtitle1' sx={{ color: "navbar.primary", fontWeight: 500 }}>{fractionsToSell}</Typography>
                                                <svg onClick={() => {
                                                    if (fractionsToSell > 1) {
                                                        setFractionsToSell(fractionsToSell - 1)
                                                        setSliderValue(fractionsToSell - 1)
                                                    }
                                                }} style={{ cursor: "pointer" }} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <Box sx={{ my: 6 }}>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 3, borderBottom: 1, borderBottomColor: 'assetPurchase.documentRedirectBorder', pb: 0.5 }}>
                                            ...
                                            <Typography variant='body1' sx={{ color: "navbar.primary", fontWeight: 500 }}>{fractionsToSell}</Typography>
                                        </Box>
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 2, borderBottom: 1, borderBottomColor: 'assetPurchase.documentRedirectBorder', pb: 0.5 }}>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                <Typography variant='subtitle1' sx={{ color: "navbar.primary", fontWeight: 500 }}>{fractionsToSell * Number(fractionPrice)}</Typography>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <Button
                                            ...
                                            <Typography variant='h6' sx={{ color: "navbar.primary", fontWeight: 500 }}>List {fractionsToSell} {fractionsToSell > 1 ? "Fractions" : "Fraction"}</Typography>
